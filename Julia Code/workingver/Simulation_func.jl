...
## Function that generates state transition markov chains (multivariate markov chains)
    ...
    # Alternative version (price/productivity)
    function StateTransition_func_multivariate_alt(p::Par,M::Model)
        @unpack p,ngrid,nstate=M;
        # Discretization of VAR(1) where matrix of impact coefficient is diagonal matrix of individual AR(1) coefficients.
        # Starting with OE
            b = zeros(nstate);                                                  # constant
            ρ_state = [p.ρz,p.ρ_pm,p.ρ_e,p.ρ_ψo,0,0];                           # persistence of state variables in order: z,pm,pe/ψe,ψo,0,0
            B = diagm(ρ_state);                                                 # Diagonalize persistence matrix
            Cov = Array(p.p_cov_all);                                           # Variance/covariance matrix of shocks
            MP_state = discrete_var(b, B, Cov, ngrid,2,Even(),sqrt(ngrid-1));   # Markov chain for all state variables
            Πs_oe = MP_state.p;                                                 # Transition matrix
            lnSgrid_oe = mapreduce(permutedims,vcat,MP_state.state_values);     # grid in logs
            Sgrid_oe = exp.(lnSgrid_oe);                                        # grid in levels
        # Starting with OGE
            b = zeros(nstate);                                                  # constant
            ρ_state = [p.ρz,p.ρ_pm,p.ρ_e,p.ρ_ψo,p.ρ_g,0];                       # persistence of state variables in order: z,pm,pe/ψe,ψo,pg/ψg,0
            B = diagm(ρ_state);                                                 # Diagonalize persistence matrix
            Cov = Array(p.p_cov_all);                                           # Variance/covariance matrix of shocks
            MP_state = discrete_var(b, B, Cov, ngrid,2,Even(),sqrt(ngrid-1));   # Markov chain for all state variables
            Πs_oge = MP_state.p;                                                # Transition matrix
            lnSgrid_oge = mapreduce(permutedims,vcat,MP_state.state_values);    # grid in logs
            Sgrid_oge = exp.(lnSgrid_oge);                                      # grid in levels
        # Starting with OCE
            b = zeros(nstate);                                                  # constant
            ρ_state = [p.ρz,p.ρ_pm,p.ρ_e,p.ρ_ψo,0,p.ρ_c];                       # persistence of state variables in order: z,pm,pe/ψe,ψo,0,pc/ψc
            B = diagm(ρ_state);                                                 # Diagonalize persistence matrix
            Cov = Array(p.p_cov_all);                                           # Variance/covariance matrix of shocks
            MP_state = discrete_var(b, B, Cov, ngrid,2,Even(),sqrt(ngrid-1));   # Markov chain for all state variables
            Πs_oce = MP_state.p;                                                # Transition matrix
            lnSgrid_oce = mapreduce(permutedims,vcat,MP_state.state_values);    # grid in logs
            Sgrid_oce = exp.(lnSgrid_oce);                                      # grid in levels
        # Starting with OGCE
            b = zeros(nstate);                                                  # constant
            ρ_state = [p.ρz,p.ρ_pm,p.ρ_e,p.ρ_ψo,p.ρ_g,p.ρ_c];                   # persistence of state variables in order: z,pm,pe/ψe,ψo,pg/ψg,pc/ψc
            B = diagm(ρ_state);                                                 # Diagonalize persistence matrix
            Cov = Array(p.p_cov_all);                                           # Variance/covariance matrix of shocks
            MP_state = discrete_var(b, B, Cov, ngrid,2,Even(),sqrt(ngrid-1));   # Markov chain for all state variables
            Πs_ogce = MP_state.p;                                               # Transition matrix
            lnSgrid_ogce = mapreduce(permutedims,vcat,MP_state.state_values);   # grid in logs
            Sgrid_ogce = exp.(lnSgrid_ogce);                                    # grid in levels
        #
        # Update model 
        M = Model(M; Πs_oe=copy(Πs_oe),lnSgrid_oe=copy(lnSgrid_oe),Sgrid_oe=copy(Sgrid_oe),
                    Πs_oge=copy(Πs_oge),lnSgrid_oge=copy(lnSgrid_oge),Sgrid_oge=copy(Sgrid_oge),
                    Πs_oce=copy(Πs_oce),lnSgrid_oce=copy(lnSgrid_oce),Sgrid_oce=copy(Sgrid_oce),
                    Πs_ogce=copy(Πs_ogce),lnSgrid_ogce=copy(lnSgrid_ogce),Sgrid_ogce=copy(Sgrid_ogce));
        return M;
    end
#
...
## Functions that find the closest point on the grid to each observed state variable from the data
    ...
    # Version with price/productivity as states
    function MatchGridData_pre_alt(M,Data::DataFrame)
        @unpack p = M;
        NT = size(Data,1);
        ## Find closest point on the grid for each state variable (In order, state variables are: z,pm,pe,ψe,ψo,c,g)
        # Keeping track of grid indices for each firm
        ...
        ind_cre = convert.(Int64, zeros(NT)) ;
        ind_s = convert.(Int64, zeros(NT))  ;
        grid_indices = DataFrame(s = ind_s, g_re = ind_gre, c_re = ind_cre) ;
        dist_grid_smallest = zeros(NT);
        # Find smallest grid points by euclidean distance
        dist_grid_smallest = zeros(NT);
        z = Data.res_lnz;
        pm = Data.res_pm;
        peψe = Data.res_peprode;
        ψo = Data.res_prodo
        pgψg = Data.res_pgprodg;
        pcψc = Data.res_pcprodc;
        state_oe = [z[Data.combineF.==12] pm[Data.combineF.==12] peψe[Data.combineF.==12] ψo[Data.combineF.==12]];
        state_oge = [z[Data.combineF.==124] pm[Data.combineF.==124] peψe[Data.combineF.==124] ψo[Data.combineF.==124] pgψg[Data.combineF.==124]];
        state_oce = [z[Data.combineF.==123] pm[Data.combineF.==123] peψe[Data.combineF.==123] ψo[Data.combineF.==123] pcψc[Data.combineF.==123]];
        state_ogce = [z[Data.combineF.==1234] pm[Data.combineF.==1234] peψe[Data.combineF.==1234] ψo[Data.combineF.==1234] pgψg[Data.combineF.==1234] pcψc[Data.combineF.==1234]];
        # L-2 norm between observed state variables and each combination possible on the grid
        dist_grid_oe = pairwise(euclidean, state_oe', M.lnSgrid_ogce[:,1:4]', dims=2);
        dist_grid_oge = pairwise(euclidean, state_oge', M.lnSgrid_ogce[:,1:5]', dims=2);
        dist_grid_oce = pairwise(euclidean, state_oce', M.lnSgrid_ogce[:,[1,2,3,4,6]]', dims=2);
        dist_grid_ogce = pairwise(euclidean, state_ogce', M.lnSgrid_ogce', dims=2);
        dist_grid = vcat(dist_grid_oe,dist_grid_oce,dist_grid_oge,dist_grid_ogce);
        grid_indices.s .= getindex.(argmin(dist_grid,dims=2),2);
        for i = 1:NT
            dist_grid_smallest[i] = dist_grid[grid_indices.s[i]]
            # Comparative advantage for coal
            if ismissing(Data.lnpc_prodc_re[i]) == false
                grid_indices.c_re[i] = searchsortednearest(M.lnc_re_grid[:],Data.lnpc_prodc_re[i]) ;
            end
            # Comparative advantage for gas
            if ismissing(Data.lnpg_prodg_re[i]) == false
                grid_indices.g_re[i] = searchsortednearest(M.lng_re_grid[:],Data.lnpg_prodg_re[i]) ;
            end
        end
        return grid_indices, dist_grid_smallest;
    end
...
### Functions that compute per-period profits
    # Pre-estimation: Compute Static Profits Next Period under all combination of state variables
        ...
        # with fuel price/productivity as state variable
        function StaticProfit_grid_pre_alt(M::Model)
            @unpack p,ngrid,nstate,nc_re,ng_re = M;
            @unpack rk,w,po,pout_struc,Ygmean = p;
            # Initialize static profit under grid points (order of states: z,pm,pe,ψe,ψo)
            ...
            @time for k = 1:(p.F_tot*p.T*p.R*ngrid^nstate*nc_re*ng_re)
                f,t,r,is,ic_re,ig_re = Tuple(CartesianIndices((p.F_tot,p.T,p.R,ngrid^nstate,nc_re,ng_re))[k]);
                # State variables based on starting fuel set
                if f == 1 # OE
                    peψe = exp(M.lnSgrid_oe[is,3]+p.μe_t[t+1]+p.μe_r[r]);
                    poψo = (p.po[t+1])/exp(M.lnSgrid_oe[is,4]+p.μψo_t[t+1]+p.μψo_r[r]);
                    pgψg = exp(M.lnSgrid_oe[is,5]+p.μg_t[t+1]+p.μg_r[r] + M.lng_re_grid[ig_re]);
                    pcψc = exp(M.lnSgrid_oe[is,6]+p.μc_t[t+1]+p.μc_r[r] + M.lnc_re_grid[ic_re]);
                    pm = exp(M.lnSgrid_oe[is,2]+p.μpm_t[t+1]+p.μpm_r[r]); 
                    z = exp(M.lnSgrid_oe[is,1]+p.μz_t[t+1]+p.μz_r[r]);
                elseif f == 2 # OGE
                    peψe = exp(M.lnSgrid_oge[is,3]+p.μe_t[t+1]+p.μe_r[r]);
                    poψo = (p.po[t+1])/exp(M.lnSgrid_oge[is,4]+p.μψo_t[t+1]+p.μψo_r[r]);
                    pgψg = exp(M.lnSgrid_oge[is,5]+p.μg_t[t+1]+p.μg_r[r] + M.lng_re_grid[ig_re]);
                    pcψc = exp(M.lnSgrid_oge[is,6]+p.μc_t[t+1]+p.μc_r[r] + M.lnc_re_grid[ic_re]);
                    pm = exp(M.lnSgrid_oge[is,2]+p.μpm_t[t+1]+p.μpm_r[r]); 
                    z = exp(M.lnSgrid_oge[is,1]+p.μz_t[t+1]+p.μz_r[r]);
                elseif f == 3 # OCE
                    peψe = exp(M.lnSgrid_oce[is,3]+p.μe_t[t+1]+p.μe_r[r]);
                    poψo = (p.po[t+1])/exp(M.lnSgrid_oce[is,4]+p.μψo_t[t+1]+p.μψo_r[r]);
                    pgψg = exp(M.lnSgrid_oce[is,5]+p.μg_t[t+1]+p.μg_r[r] + M.lng_re_grid[ig_re]);
                    pcψc = exp(M.lnSgrid_oce[is,6]+p.μc_t[t+1]+p.μc_r[r] + M.lnc_re_grid[ic_re]);
                    pm = exp(M.lnSgrid_oce[is,2]+p.μpm_t[t+1]+p.μpm_r[r]); 
                    z = exp(M.lnSgrid_oce[is,1]+p.μz_t[t+1]+p.μz_r[r]);
                elseif f == 4 # OGCE
                    peψe = exp(M.lnSgrid_ogce[is,3]+p.μe_t[t+1]+p.μe_r[r]);
                    poψo = (p.po[t+1])/exp(M.lnSgrid_ogce[is,4]+p.μψo_t[t+1]+p.μψo_r[r]);
                    pgψg = exp(M.lnSgrid_ogce[is,5]+p.μg_t[t+1]+p.μg_r[r] + M.lng_re_grid[ig_re]);
                    pcψc = exp(M.lnSgrid_ogce[is,6]+p.μc_t[t+1]+p.μc_r[r] + M.lnc_re_grid[ic_re]);
                    pm = exp(M.lnSgrid_ogce[is,2]+p.μpm_t[t+1]+p.μpm_r[r]); 
                    z = exp(M.lnSgrid_ogce[is,1]+p.μz_t[t+1]+p.μz_r[r]);
                end
                pfψf_oe = [poψo,peψe]                                                                   ;
                pfψf_oce = [poψo,peψe,pcψc]                                                             ;
                pfψf_oge = [poψo,peψe,pgψg]                                                             ;
                ...
                pin_oge = pinput_func(rk[t+1],pm,w[t+1],pE_oge,p);
                pin_ogce = pinput_func(rk[t+1],pm,w[t+1],pE_ogce,p);
                πgrid_oe[f,t,r,is,ic_re,ig_re] = profit_func_monopolistic(z,pin_oe,t,p);
                πgrid_oge[f,t,r,is,ic_re,ig_re] = profit_func_monopolistic(z,pin_oge,t,p);
                πgrid_oce[f,t,r,is,ic_re,ig_re] = profit_func_monopolistic(z,pin_oce,t,p);
                πgrid_ogce[f,t,r,is,ic_re,ig_re] = profit_func_monopolistic(z,pin_ogce,t,p);
            end
            # Update model
            M=Model(M; πgrid_oe = copy(convert(Array,πgrid_oe)), πgrid_oge = copy(convert(Array,πgrid_oge)), πgrid_oce = copy(convert(Array,πgrid_oce)), πgrid_ogce = copy(convert(Array,πgrid_ogce)));
            return M;
